...
On 2026-09-25, EAS confirmed that `SENTRY_AUTH_TOKEN` is present in the HireQuick project's **preview and production** environments. Its value was not displayed or retrieved; token permissions and successful uploads still need verification in a native build. Keep this organization upload token under secret visibility. Do not add it to `app.json`, `eas.json`, Git, or an `EXPO_PUBLIC_*` variable. The `ios-archive` profile inherits preview configuration and uses its environment.
