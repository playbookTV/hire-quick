...
/** Event times are Lagos wall-clock times (UTC+1), with no overnight events. */
export function eventInstant(date: Date, time: string): Date {
...
export function staffingStatus(
  ...
  if (now >= eventInstant(event.eventDate, event.endTime) && bookings.every((s) => TERMINAL_BOOKING_STATUSES.includes(s)))
    return 'COMPLETED';
  if (event.status === 'IN_PROGRESS' || now >= eventInstant(event.eventDate, event.startTime)) return 'IN_PROGRESS';
  const occupied = bookings.filter((s) => !VACATED_BOOKING_STATUSES.includes(s)).length;
  return occupied >= event.headcount ? 'FULLY_STAFFED' : occupied > 0 ? 'PARTIALLY_STAFFED' : 'OPEN';
}

/** Caller holds the event lock; every booking status writer uses this hierarchy. */
export async function refreshEventStaffing(tx: Tx, eventId: string, now = new Date()): Promise<void> {
  const event = await tx.event.findUniqueOrThrow({ where: { id: eventId } });
  const bookings = await tx.booking.findMany({ where: { eventId }, select: { status: true } });
  const status = staffingStatus(event, bookings.map((b) => b.status), now);
...
export async function lockBookingLifecycle(tx: Tx, bookingId: string) {
  const identity = await tx.booking.findUniqueOrThrow({ where: { id: bookingId }, select: { eventId: true, orderId: true } });
  await tx.$queryRaw`SELECT id FROM events WHERE id = ${identity.eventId}::uuid FOR UPDATE`;
  if (identity.orderId) await tx.$queryRaw`SELECT id FROM orders WHERE id = ${identity.orderId}::uuid FOR UPDATE`;
...
export async function lockOrderLifecycle(tx: Tx, orderId: string): Promise<void> {
  const order = await tx.order.findUniqueOrThrow({ where: { id: orderId }, select: { eventId: true } });
