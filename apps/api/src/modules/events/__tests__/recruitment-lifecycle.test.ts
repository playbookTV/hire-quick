...
import { cancelBooking, holdOrder, markCheckedIn, refundBooking, releaseBooking } from '../../payments/ledger/ledger.js';
import { InMemoryPaystack } from '../../payments/port/paystack-port.js';
import { lockBookingLifecycle } from '../staffing.js';
import { applyToEvent, inviteToEvent, replyToInvitation, selectApplication } from '../recruitment.js';
...
afterEach(async () => {
  ...
  await prisma.paymentOperation.deleteMany({ where: { dedupeKey: { in: bookingIds.map((id) => `BOOKING_REFUND:${id}`) } } });
...
async function fixture() {
  const client = await prisma.user.create({ data: { phone: `recruit-client-${randomUUID()}`, role: 'CLIENT', status: 'ACTIVE', client: { create: { displayName: 'Recruitment client' } } }, include: { client: true } });
  const usher = await prisma.user.create({ data: { phone: `recruit-usher-${randomUUID()}`, role: 'USHER', status: 'ACTIVE', usher: { create: { verificationStatus: 'VERIFIED', wallet: { create: {} } } } }, include: { usher: true } });
  users.push(client.id, usher.id);
  const event = await prisma.event.create({ data: { clientId: client.client!.id, title: 'Recruitment event', venue: 'Lagos venue', category: 'Wedding', eventDate: new Date('2030-06-01'), startTime: '10:00', endTime: '18:00', headcount: 1, budgetPerHead: 10000, status: 'OPEN' } });
  ...
  const params = { idempotencyKey: key, clientUserId: client.id, eventId: event.id, applicationIds: [application.id], email: 'recruit@example.com' };
  const paystack = new InMemoryPaystack();
  const initialize = vi.spyOn(paystack, 'initializeCharge');
  return { client, usher, usherId: usher.usher!.id, clientId: client.client!.id, event, application, params, paystack, initialize, deps: { prisma, paystack } };
}
...
describe('locked recruitment and staffing lifecycle', () => {
  it('rejects apply to closed or started events without changing the prior application', async () => {
    ...
    for (const status of ['DRAFT', 'FULLY_STAFFED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED'] as EventStatus[]) {
      await prisma.event.update({ where: { id: f.event.id }, data: { status } });
      await expect(applyToEvent(prisma, f.event.id, f.usherId)).rejects.toMatchObject({ code: 'EVENT_LOCKED' });
    }
    await prisma.event.update({ where: { id: f.event.id }, data: { status: 'OPEN', eventDate: new Date('2020-01-01') } });
    const api = createApp({});
    const response = await request(api).post(`/api/events/${f.event.id}/apply`).set('Authorization', `Bearer ${await signAccessToken(f.usher.id, 'USHER')}`).send({});
    expect(response.status).toBe(409);
    expect((await prisma.application.findUniqueOrThrow({ where: { id: f.application.id } })).status).toBe('ACCEPTED');
  });
  ...
  it('rechecks client suspension, staff suspension, and verification before selection and checkout', async () => {
    ...
    await expect(selectApplication(prisma, f.application.id, f.clientId, 'ACCEPTED')).rejects.toMatchObject({ code: 'SELECTION_CHANGED' });
    await prisma.user.update({ where: { id: f.usher.id }, data: { status: 'ACTIVE' } });
    await prisma.usher.update({ where: { id: f.usherId }, data: { verificationStatus: 'REJECTED' } });
    await expect(inviteToEvent(prisma, f.event.id, f.clientId, f.usherId)).rejects.toMatchObject({ code: 'SELECTION_CHANGED' });
    await expect(confirmBatch(f.deps, f.params)).rejects.toMatchObject({ code: 'SELECTION_CHANGED' });
  ...
  it('serializes different clients booking the same usher across overlapping events', async () => {
    ...
    await prisma.application.update({ where: { id: b.application.id }, data: { usherId: a.usherId } });
    const results = await Promise.allSettled([confirmBatch(a.deps, a.params), confirmBatch(b.deps, b.params)]);
  ...
  it('allows adjacent intervals but rejects unavailable dates before payment dispatch', async () => {
    const a = await fixture(), b = await fixture();
    await prisma.application.update({ where: { id: b.application.id }, data: { usherId: a.usherId } });
    await prisma.event.update({ where: { id: b.event.id }, data: { startTime: '18:00', endTime: '20:00' } });
    await prisma.availability.create({ data: { usherId: a.usherId, date: a.event.eventDate, status: 'UNAVAILABLE' } });
    await expect(confirmBatch(a.deps, a.params)).rejects.toMatchObject({ code: 'SCHEDULE_CONFLICT' });
    expect(a.initialize).not.toHaveBeenCalled();
    await prisma.availability.update({ where: { usherId_date: { usherId: a.usherId, date: a.event.eventDate } }, data: { status: 'AVAILABLE' } });
  ...
  it('serializes availability insertion with checkout so only one incompatible intent succeeds', async () => {
    ...
    const [confirmation, availability] = await Promise.allSettled([
      ...
      request(api).put('/api/me/availability').set('Authorization', `Bearer ${token}`).send({ date: '2030-06-01', status: 'UNAVAILABLE' }),
    ]);
    ...
    if (confirmation.status === 'fulfilled') {
      ...
      expect(await prisma.availability.findUnique({ where: { usherId_date: { usherId: f.usherId, date: f.event.eventDate } } })).toBeNull();
    } else {
  ...
  it('accept/decline race leaves a successful decline withdrawn and impossible to reselect or reinvite', async () => {
    ...
    expect((await prisma.invitation.findUniqueOrThrow({ where: { id: invitation.id } })).status).toBe('DECLINED');
    expect((await prisma.application.findUniqueOrThrow({ where: { id: f.application.id } })).status).toBe('WITHDRAWN');
    await expect(replyToInvitation(prisma, invitation.id, f.usherId, 'ACCEPTED')).rejects.toMatchObject({ code: 'INVITATION_CLOSED' });
    await expect(inviteToEvent(prisma, f.event.id, f.clientId, f.usherId)).rejects.toMatchObject({ code: 'INVITATION_CLOSED' });
    await expect(applyToEvent(prisma, f.event.id, f.usherId)).rejects.toMatchObject({ code: 'INVITATION_CLOSED' });
    await expect(selectApplication(prisma, f.application.id, f.clientId, 'ACCEPTED')).rejects.toMatchObject({ code: 'INVITATION_CLOSED' });
    await expect(confirmBatch(f.deps, f.params)).rejects.toMatchObject({ code: 'SELECTION_CHANGED' });
    expect(f.initialize).not.toHaveBeenCalled();
  });
  ...
  it('decline versus confirm permits either booking or withdrawal, never both', async () => {
    ...
    const [decline, confirmation] = await Promise.allSettled([
      replyToInvitation(prisma, invitation.id, f.usherId, 'DECLINED'), confirmBatch(f.deps, f.params),
    ]);
    ...
    if (decline.status === 'fulfilled') {
      ...
      expect((await prisma.invitation.findUniqueOrThrow({ where: { id: invitation.id } })).status).toBe('ACCEPTED');
    }
  });
  ...
  it('replays acceptance without resurrecting client rejection and repairs legacy decline application state', async () => {
    ...
    await selectApplication(prisma, f.application.id, f.clientId, 'REJECTED');
    await replyToInvitation(prisma, invitation.id, f.usherId, 'ACCEPTED');
    expect((await prisma.application.findUniqueOrThrow({ where: { id: f.application.id } })).status).toBe('REJECTED');
    await prisma.invitation.update({ where: { id: invitation.id }, data: { status: 'DECLINED' } });
    await prisma.application.update({ where: { id: f.application.id }, data: { status: 'ACCEPTED' } });
  ...
  it('rejects invitation acceptance after the event closes and checks invitation ownership', async () => {
    ...
    const { invitation } = await inviteToEvent(prisma, f.event.id, f.clientId, f.usherId);
    await expect(replyToInvitation(prisma, invitation.id, foreign.usherId, 'ACCEPTED')).rejects.toMatchObject({ code: 'FORBIDDEN' });
    await prisma.event.update({ where: { id: f.event.id }, data: { status: 'CANCELLED' } });
    await expect(replyToInvitation(prisma, invitation.id, f.usherId, 'ACCEPTED')).rejects.toMatchObject({ code: 'EVENT_LOCKED' });
    expect((await prisma.invitation.findUniqueOrThrow({ where: { id: invitation.id } })).status).toBe('SENT');
  });
  ...
  it('reopens a future fully staffed event atomically with its terminal refund', async () => {
    ...
    expect((await prisma.booking.findUniqueOrThrow({ where: { id: out.bookingIds[0]! } })).status).toBe('REFUNDED');
  });

  it('time sweep completes empty and early-paid events only after their end', async () => {
    const empty = await fixture(), paid = await fixture();
    const out = await confirmBatch(paid.deps, paid.params);
    await prisma.$transaction((tx) => holdOrder(tx, out.orderId, out.reference), TX);
    await prisma.$transaction(async (tx) => {
      await markCheckedIn(tx, out.bookingIds[0]!, 'OTP');
      await releaseBooking(tx, out.bookingIds[0]!, 'OTP');
    }, TX);
    expect((await prisma.event.findUniqueOrThrow({ where: { id: paid.event.id } })).status).toBe('FULLY_STAFFED');
    await autoComplete(new Date('2030-06-01T09:00:00Z'));
    expect((await prisma.event.findUniqueOrThrow({ where: { id: empty.event.id } })).status).toBe('IN_PROGRESS');
  ...
  it('re-reads a check-in that wins the lifecycle lock before auto-completion', async () => {
    ...
    expect((await prisma.booking.findUniqueOrThrow({ where: { id: bookingId } })).status).toBe('PAID');
  });

  it('a reserved refund skips payout without starving completion of an unrelated empty event', async () => {
    const f = await fixture(), empty = await fixture();
    const out = await confirmBatch(f.deps, f.params);
    const bookingId = out.bookingIds[0]!;
    await prisma.$transaction((tx) => holdOrder(tx, out.orderId, out.reference), TX);
    await prisma.booking.update({ where: { id: bookingId }, data: { arrivalAssertedAt: new Date() } });
    await prisma.paymentOperation.create({ data: { kind: 'BOOKING_REFUND', dedupeKey: `BOOKING_REFUND:${bookingId}`, payload: { bookingId, amountKobo: 10000, precursor: 'CANCEL', chargeReference: out.reference } } });
    ...
    expect((await prisma.event.findUniqueOrThrow({ where: { id: empty.event.id } })).status).toBe('COMPLETED');
  });
  ...
  it('repairs legacy future fully staffed events whose bookings already vacated their slots', async () => {
    ...
    await prisma.booking.update({ where: { id: out.bookingIds[0]! }, data: { status: 'CANCELLED' } });
    expect((await prisma.event.findUniqueOrThrow({ where: { id: f.event.id } })).status).toBe('FULLY_STAFFED');
