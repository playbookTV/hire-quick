...
async function login(
  ...
  const r2 = await request(app)
    ...
    .send({ phone, code: r1.body.devCode, role })
    .timeout({ response: 30000, deadline: 45000 });
...
async function setupConfirmedBooking(opts: {
  ...
  const usherRow = await prisma.usher.findFirstOrThrow({
    where: { userId: usher.userId },
    include: { wallet: true },
  });
  await prisma.usher.update({
    where: { id: usherRow.id },
  ...
  const apply = await request(app)
    ...
    .set('Authorization', `Bearer ${usher.token}`)
    .send({})
    .timeout({ response: 30000, deadline: 45000 });
  ...
  await prisma.event.update({
    where: { id: eventId },
    data: { eventDate: new Date(opts.eventDate) },
  });
  ...
  return {
    ...
    usherToken: usher.token,
    eventId,
    bookingId,
    walletId: usherRow.wallet!.id,
  };
}

describe('full booking lifecycle (TRD §7/§8/§12)', () => {
  it('HTTP completion holds earnings through the dispute window before worker wallet release', async () => {
    const yesterday = new Date(Date.now() - 86_400_000).toISOString().slice(0, 10);
    const s = await setupConfirmedBooking({
      eventDate: yesterday,
      startTime: '10:00',
      endTime: '18:00',
    });
    ...
    const detail = await request(app)
      .get(`/api/bookings/${s.bookingId}`)
      .set('Authorization', `Bearer ${s.clientToken}`);
    expect(detail.status).toBe(200);
    expect(detail.body.canDispute).toBe(true);
    expect(detail.body.payment.escrowStatus).toBe('HELD');
    const deadline = new Date(detail.body.payoutAvailableAt);
    expect(deadline.getTime()).toBe(new Date(`${yesterday}T17:00:00Z`).getTime() + 72 * 3_600_000);
    await autoComplete(deadline);
