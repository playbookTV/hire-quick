...
describe('durable capacity-blocked withdrawal reversals', () => {
  it('keeps a late reversal retryable and credits exactly once after capacity becomes available', async () => {
    ...
    expect(intent).toMatchObject({ status: 'PROVIDER_OK', lastError: expect.stringContaining('BALANCE_LIMIT') });
    ...
    expect((await prisma.paymentOperation.findUniqueOrThrow({ where: { id: intent.id } })).status).toBe('RECORDED');
  }, 120_000);
