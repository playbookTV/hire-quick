...
import { readOrderOutcome } from '../../lib/order-checkout.js';
import { money } from '../../lib/format.js';

export default function FundsHeld(): React.JSX.Element {
  const router = useRouter();
  const { event: eventId = '', order: orderId = '' } = useLocalSearchParams<{
    event?: string;
    order?: string;
  }>();
  const { user } = useAuth();
  ...
  const [displayed, setDisplayed] = useState<{ scope: string; value: CheckoutResponse } | null>(
    null,
  );
  const outcome = displayed?.scope === scope ? displayed.value : null;
  const setOutcome = useCallback(
    (value: CheckoutResponse) => setDisplayed({ scope, value }),
    [scope],
  );
  useEffect(() => {
    fence.activate(scope);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (current()) setError(e instanceof Error ? e.message : 'Payment status is unavailable.');
    } finally {
  ...
  useEffect(() => {
    ...
    void checkoutStore
      ...
      .catch((e: unknown) => {
        if (current()) setError(e instanceof Error ? e.message : 'Cannot read saved checkout.');
      });
    return () => {
      fence.invalidate();
    };
  ...
  const resume = async (): Promise<void> => {
    ...
    setError('');
    try {
      const saved = orderId ? null : await checkoutStore.load(user.id, eventId);
      if (!current()) return;
      if (!orderId && !saved) throw new Error('No saved checkout.');
      const result = orderId
        ? readOrderOutcome(
            orderId,
            await api.post(`/api/payments/orders/${orderId}/checkout/resume`),
          )
        : await checkoutStore.submit(user.id, eventId, saved!.input);
      if (!current()) return;
      setOutcome(result);
      if (result.state === 'READY' && result.authorizationUrl)
        await WebBrowser.openBrowserAsync(result.authorizationUrl);
      if (!current()) return;
      const checked = orderId
        ? readOrderOutcome(orderId, await api.get(`/api/payments/orders/${orderId}/checkout`))
        : await checkoutStore.refresh(user.id, eventId);
      if (current()) setOutcome(checked);
    } catch (e) {
      if (current()) setError(e instanceof Error ? e.message : 'Could not resume checkout.');
    } finally {
      if (current()) setBusy(false);
    }
  };
  const done = async (): Promise<void> => {
    ...
    try {
      ...
      if (current()) setError(e instanceof Error ? e.message : 'Could not acknowledge checkout.');
    } finally {
      if (current()) setBusy(false);
    }
  };
      ...
      <Screen scroll>
        <Box style={{ gap: 20, paddingTop: 32 }}>
          {copy ? (
            ...
            <Text variant="body">
              Your saved checkout will appear here when its status is available.
            </Text>
          )}
          {error ? (
            <Text variant="bodySm" color="inkMuted">
          ...
          {outcome && ['CREATED', 'READY'].includes(outcome.state) ? (
            <Button
              label={orderId ? 'Review and resume payment' : 'Resume Paystack'}
              disabled={busy}
              onPress={() => {
                if (orderId)
                  router.push({
                    pathname: '/(modals)/payment-summary',
                    params: { id: outcome.eventId, order: orderId },
                  });
                else void resume();
              }}
            />
