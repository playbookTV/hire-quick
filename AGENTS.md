...
HireQuick — a mobile-first staffing marketplace (Lagos event ushers) where clients book and pay for event staff, funds are held in **escrow until verified attendance**, and the platform takes a 15% commission. This repo is the backend monorepo. The mobile/admin apps are not built yet; `apps/api` is the only app.
...
### Workspace layout
- **`packages/shared` (`@hq/shared`)** — pure, dependency-free domain logic: `money.ts` (kobo + fee math), `policy.ts` (cancellation/no-show matrix), `state-machines.ts` (transition tables), `enums.ts`, `dto.ts` (Zod DTOs). Imported by everything; the future mobile/admin apps will import the same matrices so all surfaces compute identical outcomes.
...
- **`apps/api` (`@hq/api`)** — Express (ESM) HTTP service. Feature modules under `src/modules/*` (`auth`, `payments`, `bookings`, `profile`, `admin`, `audit`).
